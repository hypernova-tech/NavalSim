...
bool CUtil::Trace(STraceArgs& args, SScanResult* pscan_result)
{
#if true
    ...
    FVector actor_rpy_deg = CMath::GetActorEulerAnglesRPY(args.p_actor);
    ...
    if (p_include_actor_list) {
        for (auto pactor : *p_include_actor_list) {
            ...
            for (auto corner : corners) {
                ...
                if (CMath::IsPointInsideVolume(!args.is_world, start_loc, look_dir, corner, args.elevation_start_deg, args.elevation_end_deg, args.azimuth_start_deg, args.azimuth_end_deg, args.min_range_meter, args.range_meter, local_rpy))
                {
    ...
    if (args.include_point_list.Num() > 0) {
        ...
        for (FVector corner : args.include_point_list) {
            ...
            if (CMath::IsPointInsideVolume(!args.is_world, start_loc, look_dir, corner, args.elevation_start_deg , args.elevation_end_deg , args.azimuth_start_deg , args.azimuth_end_deg , args.min_range_meter, args.range_meter, local_rpy))
            {
                ...
                corner_horizantal_ind = (corner_rpy.Z - (args.azimuth_start_deg )) / args.azimuth_angle_step_deg;
                corner_vertical_ind = (corner_rpy.Y - (args.elevation_start_deg )) / args.elevation_angle_step_deg;
                args.drived_params.visibility_check_location = corner;
    ...
    if (args.is_world) {
        ...
        */
        ...
        if (!is_visiblity) {
            ...
            end = args.drived_params.visibility_check_location;
        }
...
void CUtil::DebugSphere(UWorld* p_world, const FVector& Center, float Radius, FColor Color, float Duration)
{
    const int32 Segments = 8; // Number of segments to use for drawing the sphere
     const bool bPersistentLines = false; // Whether the lines should persist after the frame refreshes
    const float LifeTime = Duration; // How long the sphere should be visible (-1 means forever, during the play session)
    const uint8 DepthPriority = 0; // Priority for rendering order, lower numbers render on top

    // Draw the sphere
    DrawDebugSphere(
        p_world, // World context
        Center, // Location in the world
        Radius, // Radius of the sphere
        Segments, // Number of segments
        Color, // Color of the sphere
        bPersistentLines, // Persistent lines
        LifeTime, // How long it lasts
        DepthPriority // Render priority
    );
}

void CUtil::DebugBox(UWorld* p_world, const FVector& Center, float Size, FColor Color, float Duration)
{
    DrawDebugBox(p_world, Center, FVector::One()*Size, Color, false, Duration, 0, 0);

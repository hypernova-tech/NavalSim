...
public:
	...
	bool IsPointInsideVolume(const FVector& origin, const FVector& point, FLOAT64 min_elevation_deg, FLOAT64 max_elevation_deg, FLOAT64 min_azimuth_deg, FLOAT64 max_azimuth_deg, FLOAT64 max_range_meter);
	FLOAT64 GetAngleFromXAxisInDegrees(const FVector& Vector);
	TArray<FVector> GetBoxCornersAndCenter(const FBox& Box);
	bool CheckBoxIndideVolume(const FBox& box, FVector& origin, FLOAT64 min_elevation_deg, FLOAT64 max_elevation_deg, FLOAT64 min_azimuth_deg, FLOAT64 max_azimuth_deg, FLOAT64 max_range_meter);
}; 